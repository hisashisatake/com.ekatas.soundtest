...
void wave_read_8bit_mono(MONO_PCM *pcm, char *file_name)
{
  ...
  pcm->s = (short*)calloc(pcm->length, sizeof(double)); /* メモリの確保 */
...
void wave_read_8bit_stereo(STEREO_PCM *pcm, char *file_name)
{
  ...
  pcm->sL = (short*)calloc(pcm->length, sizeof(double)); /* メモリの確保 */
  pcm->sR = (short*)calloc(pcm->length, sizeof(double)); /* メモリの確保 */
...
void wave_read_16bit_stereo(STEREO_PCM *pcm, char *file_name)
{
  ...
  pcm->sL = (short*)calloc(pcm->length, sizeof(short)); /* メモリの確保 */
  pcm->sR = (short*)calloc(pcm->length, sizeof(short)); /* メモリの確保 */
